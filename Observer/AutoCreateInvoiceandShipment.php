...
namespace Mavenbird\AdminPaymentMethod\Observer;
 ...
 *
 * @package Mavenbird\AdminPaymentMethod\Observer
 */
class AutoCreateInvoiceandShipment implements ObserverInterface
{
    ...
    public function execute(Observer $observer)
    {
        ...
        if ($payment->getCode() == 'adminpaymentmethod') {
            ...
            //create notified invoice and shipment by Mavenbird
     ...
     * Create invoice
     *
     * @param \Mavenbird\AdminPaymentMethod\Model\AdminPaymentMethod $payment
    ...
    private function displayNotified($order, $payment)
    {
        try {
            if ($payment->getConfigData('createinvoice') && $payment->getConfigData('createshipment')) {
                return $order->addStatusHistoryComment(__('Automatically Invoice and Shipment By Mavenbird Invoice Shipment'))->save();
            } elseif ($payment->getConfigData('createinvoice')) {
                return $order->addStatusHistoryComment(__('Automatically Invoice By Mavenbird Invoice'))->save();
            } elseif ($payment->getConfigData('createshipment')) {
                return $order->addStatusHistoryComment(__('Automatically Shipment By Mavenbird Shipment'))->save();
            }
            return null;
        } catch (\Exception $e) {
            $order->addStatusHistoryComment('Exception message: ' . $e->getMessage(), false);
